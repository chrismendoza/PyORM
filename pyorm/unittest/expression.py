...
class TestExpression(unittest.TestCase):
    ...
    def test_020(self):
        ...
        for operator_name, operator in self.algebraic_operators:
            exp = eval('Expression(3).operator(\'AND\') {0} 5'.format(operator))
            self.assertEqual(exp._operator, operator_name)
            self.assertEqual(exp._node_list, [Expression(3).operator('AND'), 5])

    def test_021(self):
        """ Expression: reverse algebraic operators return the expected expressions."""
        for operator_name, operator in self.algebraic_operators:
            exp = eval('5 {0} Expression(3).operator(\'AND\')'.format(operator))
            self.assertEqual(exp._operator, operator_name)
            self.assertEqual(exp._node_list, [5, Expression(3).operator('AND')])
    ...
    def test_040(self):
        ...
        for operator_name, operator in self.special_operators:
            ...
            self.assertEqual(exp._operator, operator_name)
            self.assertEqual(exp._node_list, [Expression(3).operator('AND'), 'testing'])

    def test_050(self):
        """ Expression: None Comparison Operators return the expected expressions."""
        for operator_name, operator in self.none_operators:
            exp = eval('Expression(3).operator(\'AND\') {0} None'.format(operator))
            self.assertEqual(exp._operator, operator_name)
            self.assertEqual(exp._node_list, [Expression(3).operator('AND'), None])
    ...
    def test_070(self):
        ...
        exp = Expression('test', Expression('cheese').set_alias('fries'), 'fish')
        exp.set_alias('testing')
        self.assertTrue(exp.get_alias('fries'))

    def test_071(self):
        """ Expression: get_alias works with unicode. """
        exp = Expression('test', Expression('cheese').set_alias(u'fries'), 'fish')
        exp.set_alias(u'testing')
    ...
    def test_090(self):
        ...
        exp = Expression('outer', Expression('test', 'cheese', 'fish').operator('ADD')).operator('SUB')
        self.assertEqual(
            exp.tokenize(),
            [
    ...
    def test_110(self):
        ...
        exp = Column.test.field1 + Column.test2.field1 + Column.test2.subtest.field1
    ...
    def test_130(self):
        ...
        exp = Expression('test', 'cheese', 'fish')
        self.assertEqual(str(exp), "(|pyorm_escape| AND |pyorm_escape| AND |pyorm_escape|)")

        exp = Expression('test', 'cheese', 'fish').operator('EQ')
        self.assertEqual(str(exp), "|pyorm_escape| EQ |pyorm_escape| EQ |pyorm_escape|")
